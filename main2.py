...
def peck_once(user: User, habit: Habit):
    ...
    if 'yes' or 'yeah' in finished:
        ...
        user.send(f"Maybe next time! Press 'start' to start {habit.name} again")
        habit.stopped = True
...
def peck_multiple(user: User, habit: Habit):
    ...
    if habit.destroyed or habit.stopped:
        return
    user.schedule_generator(habit.interval, peck_multiple(user, habit), expires_in=habit.interval, autocancel=Tru)
...
def woodpecker(user: User):
    ...
    user.send(f"Hi {name[0]}! What can i do for you? If you text 'add' we can add a new habit for you!")
    habits = []
        ...
        if 'help' in msg:
            ...
            if len(new_habit) == 0:
                ...
                if new_habit in habits:
                    user.send("looks like i already have that habit stored. to check which habits you have stored so far, text me 'habits'!")
            ...
            if 'one-time' in reminder_type:
                ...
                user.schedule_generator(habit.interval, peck_once(user, habit), expires_in=habit.interval, autocancel=True)
                ...
                habits.append(habit)
                user.schedule_generator(habit.interval, peck_multiple(user, habit), expires_in=habit.interval, autocancel=True)
                user.send(
            ...
            if habit_deletedyet == False:
                user.send("hmm... it seems like that's not one of your habits. text 'delete' to remove a habit!")
        elif 'update' in msg:
            ...
            if time_pecks[0] == 'recurring':
                ...
                for habit in habits:
                    if habit.interval != None: 
                        habList += "habit: " + habit.name + " | interval: " + str(habit.interval) + " seconds \n"
                user.send(habList)
                ...
                for habit in habits:
                    ...
                    #add new one
                        habit = Habit(user_id, habit.name, habit.new_interval[0])
                        habits.append(habit)
                    #destroy old one
                        habit.destroyed = True 
                        habits.remove(habit)
                ...
                for habit in habits:
                    ...
                    #add new one
                        ...
                        habits.append(habit)
                        user.schedule_generator(new_interval[0], peck_once(user, habit), expires_in=new_interval[0], autocancel=True)
                    #destroy old one
                        habit.destroyed = True 
                        habits.remove(habit)
                        habit_deletedyet = True 
                ...
                user.send("that's not one of the time types, silly! if you still want to delete something type 'delete' again!")
            ...
            for habit in habits:
                if habit.name == name_tomodify[0]:
                    ...
                    user.send("i updated " + habit.name + " to now remind you every " + habit.interval + " seconds!")
